...
通过深度对话帮你理清目标、制定策略的 AI 助手 —— AI 是军师，你是主公。

## 为什么做这个

大多数 AI 产品是"你问它答"，聊完就忘。军师 AI 做反向的：
...
**v2（军师模式）基础设施完成。** Dashboard + Chat + StrategyBrief + 目标提取链路已实现。

### 后续方向

- 端侧走通完整军师对话流程（StrategistPrompter 实际推理验证）
- 目标/策略的手动编辑能力
- 服务端推送提醒（依赖后端就绪）
- Watch 端触发器 + 传感器
