...
| 状态 | UI 表现 | 验收 |
|------|------|:--:|
| 模型加载中 | 启动页全屏进度条「正在准备 AI 思考引擎(~1.2GB)」 | [x] |
| 模型加载失败 | 红色错误页 + 重试按钮 | [x] |
| 等待用户输入 | 输入框可用，光标闪烁 | [x] |
| AI 思考中 | 输入框禁用，三点跳动动画 | [x] |
| 对话正常 | 气泡正常展示 | [x] |
| 网络不可用 | 顶栏黄色提示「离线模式：数据暂存本地」 | [x] |
| 洞察生成中 | 总结页 loading 动画 | [x] |
| JSON 解析失败 | 黄色提示 + 兜底纯文本展示 | [x] |

### 7.2 边界 Case

- [x] 空话题输入 → 禁用发送按钮
- [x] 超长回答（> 500 字）→ 正常截断不崩溃
- [x] 网络断连中对话 → 本地继续，网络恢复后同步
- [x] 模型推理 crash → 自动重新加载模型 + 提示用户
- [x] 快速连续发送 → 排队处理，不丢消息

### 7.3 Android 双端验证

- [x] Android 模拟器/真机正常启动
- [x] Android NNAPI 推理加速生效
- [x] 无 iOS 专属 API 调用导致的 crash
- [x] 屏幕尺寸适配（小屏 4.7" → 大屏 6.7"）
...
**验收标准**：
- [x] HuggingFace 直链正常可访问
- [x] 下载进度条实时更新百分比 + 速度
- [x] 网络中断 → 恢复后从断点继续（HTTP Range）
- [x] 下载完成 → 自动触发 llama.cpp 模型加载
- [x] 已下载完成 → 下次启动跳过下载
- [x] 下载失败 → 重试按钮 + 手动选择模型文件
- [x] 存储空间不足 → 友好提示
...
**验收标准**：
- [x] README 包含：一句话描述 + 架构图 + 技术栈 + 快速开始 + 截图（6 页面 + 错误状态）+ 简历话术
- [x] 2 分钟 Demo 视频录制完成（展示一次完整对话流程）
- [x] 6 个页面截图：话题选择、对话中、洞察总结、思维图谱、历史列表、错误状态
- [x] 代码无 debug print / TODO / 注释掉的旧代码
- [x] 仓库 Push 到 GitHub（public）
- [x] `pubspec.yaml` 描述/版本号正确
